...
fn calculate_price(map: *grid.Grid(*Cell), region: *Region) !u32 {
    ...
    for (region.cells.items) |cell| {
        const value = try is_corner(map, cell);
...
fn is_corner(map: *grid.Grid(*Cell), cell: *Cell) !bool {
    var count: u16 = 0;

    for (0..2) |y| {
        for (0..2) |x| {
            switch (get_cell_by_diff(map, cell, -x, -y)) {
                null => count += 1,
                else => |item| {
                    if (item.plant != cell.plant) count += 1;
                },
            }
        }
    }
...
fn get_cell_by_diff(map: *grid.Grid(*Cell), cell: *Cell, x_diff: i8, y_diff: i8) ?*Cell {
    const x = cell.x + x_diff;
    const y = cell.y + y_diff;

    return map.get_cell(x, y) catch null;
}
